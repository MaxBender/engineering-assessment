...
### 17. Offline simulation mode

- Added an offline simulation mode in `wiki.py` with a local page graph so the game can run without live Wikipedia API access.
- Added simulation-aware dictionary override in `main.py` so random page selection uses local simulated page titles while offline mode is enabled.
- Enable with:

```bash
WIKIBACON_SIMULATE=1 python3 main.py
```

- This mode is useful when Wikipedia API requests are rate-limited or unavailable.
